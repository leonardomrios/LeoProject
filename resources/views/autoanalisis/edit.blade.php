...
@section('title', 'Editar Reflexión - Autoanálisis')

@section('content')
<div class="card">
    <div class="card-header">
        <h2 class="card-title">Editar Reflexión</h2>
        <a href="{{ route('autoanalisis.index') }}" class="btn btn-secondary">Volver</a>
    </div>

    @php
        $preguntaTexto = App\Models\AutoanalisisRespuesta::getPreguntaTexto($autoanalisi->pregunta_numero);
        $preguntaDescripcion = App\Models\AutoanalisisRespuesta::getPreguntaDescripcion($autoanalisi->pregunta_numero);
    @endphp
    ...
    <form action="{{ route('autoanalisis.update', $autoanalisi) }}" method="POST">
        @csrf
        @method('PUT')

        <div class="form-group">
            <label for="titulo" class="form-label">Título (Opcional)</label>
            <input type="text" id="titulo" name="titulo" class="form-control" value="{{ old('titulo', $autoanalisi->titulo) }}" placeholder="Ej: Mi estrategia para aprovechar oportunidades">
        ...
        <div class="form-group">
            ...
            <textarea id="contenido" name="contenido" class="form-control" rows="12" required placeholder="Escribe aquí tu reflexión detallada sobre esta pregunta...">{{ old('contenido', $autoanalisi->contenido) }}</textarea>
        ...
        <div style="display: flex; gap: 1rem; margin-top: 2rem;">
            <button type="submit" class="btn btn-primary">Actualizar Reflexión</button>
